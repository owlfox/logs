 ...
 *
 * Compile:  gcc -g -Wall -o pth_sample pth_sample.c -pthread -lm
 ...
 *                sort:
 *
 *                   - Each thread sorts a sublist of contiguous elements
 *                     having length loc_n
 *                   - Use splitters to count number of elements going from each
 *                     thread to each thread.  These go into mat_count;
 *                     mat_count[my_rank*thread_count, th] = number of keys in
 *                     my_rank's block that should go to thread th
 */
void* Thread_work(void* rank) {
